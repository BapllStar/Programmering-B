...
# This is not meant to be a 'free lunch', I highly reccommend doing your own implementation, or at least, trying to understand EVERYTHING done in this script
# I don't wanna be doing your job, ya'know
...
import torch
import numpy as np
# Cool progress bar module
from tqdm import tqdm
import matplotlib.pyplot as plt
# Torchvision is a computervision oriented library, we only use it for their datasets and transforms
from torchvision import datasets, transforms


# torch.cuda.is_avaliable is good to use to check if you can actually use GPU
...
# Collate function is automatically called
...
class ThatMnistNet(torch.nn.Module):
    def __init__(self, in_dim, out_dim, lr=0.001, *args, **kwargs) -> None:
        ...
        self.layers = torch.nn.Sequential(
            ...
            # Torch.nn.CrossEntropyLoss automatically puts a softmax before
            ).to(device)
    ...
    #             print(self.test(test_data_loader))
...
if __name__ == "__main__":
    ...
    model = ThatMnistNet(in_dim = 28*28, out_dim=10)
    model.train(train_data_loader=train_loader, epochs=10, test_data_loader=test_loader)
